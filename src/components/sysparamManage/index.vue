  ...
  <div>
    ...
    >
      ...
      ><template v-slot="rowData" >
          <span v-html="thoseUtil.strongKeyword(rowData.row.name,tableQuery.name)" :title="rowData.row.name"></span>
      </template>
      </el-table-column>
      <el-table-column
              prop="value"
              ...
              show-overflow-tooltip
      ><template v-slot="rowData" >
        <span  :title="rowData.row.value">{{rowData.row.value}}</span>
      </template>
      </el-table-column>
      <el-table-column
              prop="value"
              ...
              show-overflow-tooltip
      ><template v-slot="rowData" >
        <span  :title="rowData.row.remark">{{rowData.row.remark}}</span>
      </template>
      </el-table-column>
      <el-table-column
        prop="alterTime"
      ...
      <el-table-column label="操作" width="100">
        <template v-slot="scope">
          <el-button
            @click="$router.push(`/sysparamManage/info/${scope.row.itemCode}`)"
            type="text"
            size="small"
            >查看</el-button
          >
...
export default {
  ...
  methods: {
    ...
    loadRemoteData() {
      ...
      this.axios
        .post(
          "/zhongan/maintain/sysparammanage/items",
