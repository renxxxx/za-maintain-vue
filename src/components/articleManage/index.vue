<template>
  <div>
    <el-row>
      <el-button style>新建</el-button>
      <el-button @click="query">查询</el-button>
    </el-row>

    <el-form style="margin-top:10px;" :inline="true" class="demo-form-inline">
      <el-form-item label="关键字:" >
        <el-input placeholder="请输入关键字" v-model="tableQuery.kw" clearable ></el-input>
      </el-form-item>
      <el-form-item label="发布时间始:">
        <el-date-picker v-model="tableQuery.addTimeStart" type="datetime" placeholder="选择日期时间"></el-date-picker>
      </el-form-item>
      <el-form-item label="发布时间止:">
        <el-date-picker v-model="tableQuery.addTimeEnd" type="datetime" placeholder="选择日期时间"></el-date-picker>
      </el-form-item>
      <el-form-item label="文章类型:">
        <el-select v-model="tableQuery.typeId" clearable placeholder="请选择">
          <el-option
            v-for="item in $store.state.articleTypes"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          ></el-option>
    ...
    >
      ...
      <el-table-column prop="cover" label="封面" width="80" align="center">
        <template v-slot="rowData">
          <img :src="rowData.row.cover" width="30" height="30">
        </template>
      </el-table-column>
      <el-table-column
        prop="typeName"
      ...
      <el-table-column label="操作" width="100">
        <template v-slot="scope">
          <el-button @click="$router.push('/index/articleManage_info')" type="text" size="small">查看</el-button>
...
export default {
  ...
  methods: {
    ...
    formatterTime(row, column, cellValue, index) {
      return this.thoseUtil.formatDate("yyyy-MM-dd hh:mm:ss", new Date(cellValue));
    },
    ...
    loadRemoteData() {
      ...
      this.axios
        .post(
          "/zhongan/mm/articlemanage/articles",
