...
export function lineToTree(items, idName, parentIdName, childrenName) {
  ...
  let am = {};
  while (true) {
    if (Object.keys(am).length == items.length) break;
    for (let i = 0; i < items.length; i++) {
      let item = items[i];
        if(am[item[idName]])
            continue;
      item[childrenName] = [];
      if (item[parentIdName] == 0) aas.push(item);
      else {
        let up = am[item[parentIdName]];
        if (!up) continue;
...
export function toLineTreeArray(
  itemsTree,
  idName,
  parentIdName,
  childrenName,
  ...
  for (let i = 0; i < itemsTree.length; i++) {
    if (!itemsTree[i].level) itemsTree[i].level = level;
    a.push(itemsTree[i]);
...
export function isVideoNamesuffix(name) {
  ...
  if (n < 0 || n == name.length - 1) return false;
  let ss = name.sub(n + 1).toLowerCase();
  if (ss == "avi" || ss == "mp4" || ss == "wmv") return true;
  else return false;
}
...
export function isImageNamesuffix(name) {
  ...
  if (n < 0 || n == name.length - 1) return false;
  let ss = name.sub(n + 1).toLowerCase();
  if (ss == "jpg" || ss == "jpeg" || ss == "png") return true;
  else return false;
}
...
export function findParent(dom, clazz) {
  if (dom.parentNode.classList.contains(clazz)) return dom.parentNode;
  else if (dom.parentNode == document.body.parentNode) return null;
  else return findParent(dom.parentNode, clazz);
}

export function strongKeyword() {
  if (arguments.length == 0) return "";
  let srcText = arguments[0];
  if (srcText == null || srcText == undefined) return srcText;
  srcText = srcText + "";
  if (arguments.length == 1) return srcText;
